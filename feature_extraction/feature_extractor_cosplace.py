...
class CosPlaceFeatureExtractor(torch.nn.Module):
    ...
    def compute_features(self, imgs: List[np.ndarray]) -> np.ndarray:
        img_set = ImageDataset(imgs)
                # NOTE: num_workers=0 avoids a known PyTorch multi-worker collate/shared-memory issue
        # that can throw: "Trying to resize storage that is not resizable" on some systems/datasets.
        # pin_memory is disabled for maximum compatibility on headless servers.
